...
for i in range(21):
    for j in range(21):
        filename = f'../tmp/profiling_ce20_de_20/profiles_local/cen_{i}_cde_0_sen_0_sde_{j}_send_lat_pod.log'
        fp = open(filename, 'r')
        lines = fp.readlines()
        fp.close()
        enter_data = False
...
xx, yy = np.meshgrid(np.linspace(0, 21), np.linspace(0, 21))
zz = model(fitted_params, xx, yy)

# Plot the fitted plane
ax.plot_surface(xx, yy, zz, alpha=1, label='Fit function', cmap='summer')
